...
当前系统只有SLAM建图和手动导航，缺少RRT自主探索的核心节点。

### 需要转换的文件

**C++ 节点（6个文件）：**
- [ ] `global_rrt_detector.cpp` - 全局RRT边界检测器
- [ ] `local_rrt_detector.cpp` - 局部RRT边界检测器
- [ ] `functions.cpp` - RRT辅助函数
- [ ] `mtrand.cpp` - 随机数生成器
- [ ] `save_map.cpp` - 地图保存（已转换）
- [ ] `rrt_save_map.cpp` - RRT地图保存（已转换）

**Python 节点（10个文件）：**
- [ ] `filter.py` - 边界点过滤器（249行）
- [ ] `assigner.py` - 目标分配器（160行）
- [ ] `frontier_opencv_detector.py` - OpenCV边界检测
- [ ] `functions.py` - Python辅助函数
- [ ] `getfrontier.py` - 边界获取
- [ ] `scan_dilute.py` - 扫描稀释（已转换）
- [ ] `send_mark.py` - 多点导航（已转换）
- [ ] `spech_send_mark.py` - 语音导航（已转换）
- [ ] `topic_relay.py` - 话题中继（已转换）
- [ ] `tf_relay.py` - TF中继（已转换）

### 转换步骤

**阶段1：Python节点转换（rospy → rclpy）**
1. 转换 `filter.py`
2. 转换 `assigner.py`
3. 转换 `frontier_opencv_detector.py`
4. 转换辅助函数

**阶段2：C++节点转换（ROS1 → ROS2 API）**
1. 转换 `global_rrt_detector.cpp`
2. 转换 `local_rrt_detector.cpp`
3. 转换辅助函数
